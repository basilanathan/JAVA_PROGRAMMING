...
//Time: O(N^2) pathSum ->O(N) pathSumUtil is being called for each node -> O(N^2)
//Space; O(N)
 ...
 * We are looking for some consecutive nodes that sum up to the given target value, which means the 
 * difference discussed in 2. should equal to the target value. In addition, we need to know how many 
 ...
 * a node in the middle of the path, such that from this node to the current node, the sum is equal to 
 * the target value.
 ...
 * Therefore, in each recursion, the map stores all information we need to calculate the number of 
 * ranges that sum up to target. Note that each range starts from a middle node, ended by the current node.
 ...
 * The interesting part of this solution is that the prefix is counted from the top(root) to 
 * the bottom(leaves), and the result of total count is calculated from the bottom to the top :D
 * 
 * */
